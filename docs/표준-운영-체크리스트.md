...
- 네이티브 최소 로딩은 `diaries/index`의 `data-diary-transition="detail"` 기반으로만 적용: `/Users/iggyu/mindlog-workspace/mindlog/src/main/resources/static/js/controllers/diary_transition_controller.js`
...
- [ ] 리스트 -> 상세 이동에서만 최소 지연(`500ms`)이 적용되는가?
- [ ] 리스트 -> 상세 이동 시 네이티브 스피너가 중첩 표시되지 않는가?
